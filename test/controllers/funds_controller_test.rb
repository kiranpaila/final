...
class FundsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create fund" do
    assert_difference('Fund.count') do
      post funds_url, params: { fund: { amount: @fund.amount, event: @fund.event, sponsor: @fund.sponsor, status: @fund.status } }
    end
  ...
  test "should update fund" do
    patch fund_url(@fund), params: { fund: { amount: @fund.amount, event: @fund.event, sponsor: @fund.sponsor, status: @fund.status } }
